...
def test_ico_integration_data_runner_pipeline() -> None:
    ...
    assert flow.children[0].node_type == "source"
    assert flow.children[1].node_type == "stream"
    ...
    assert runner_node.children[0].node_type == "chain"
    ...
    assert collate_node.node_type == "pipeline"
